layui.use([ 'form', 'table', 'treeTable' ], function() {
	...
	treeTable.on('tree(detail)', function(obj) {
		var data = obj.item ;
		xadmin.open('单位信息详情', layui.cache['contentPath'] + '/sys-menu/info/' + data.id,500,500);
	});
	// 行操作监听(编辑)
	treeTable.on('tree(edit)', function(obj) {
		var data = obj.item ;
		xadmin.open('编辑单位', layui.cache['contentPath'] + '/sys-menu/edit/' + data.id,500,500);
		obj.update({
	...
	var $ = layui.$, active = {
		...
		removeBatch : function() {
			...
			layer.confirm('确认需要删除已选中的（'+checkStatus.length+'）条组织机构记录?', function(index) {
				$.ajax({
					type : 'POST',
	...
	var renderTreeTable = function(params){
		$.post(layui.cache['contentPath'] + '/sys-org/findList',params ,function(data) {
			data = eval('('+data+')');
			// 表格数据初始化
			trees = treeTable.render({
				...
				icon_key : 'orgCode',// 必须
				primary_key : 'rwid',
				parent_key : 'parentOrgId',
				top_value : '0',
				is_click_icon: true,
				is_checkbox: true,
				cols : [ 
					{key : 'orgCode',title : '编码',width : '100px',align : 'center'}, 
					{key : 'orgName',title : '名称',width : '150px',align : 'center'}, 
					{key : 'parentOrgName',title : '上级名称',width : '150px',align : 'center'}, 
